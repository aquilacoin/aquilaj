...
import org.conciergej.core.listeners.BlockChainListener;
import org.conciergej.store.FlatDB;
import org.conciergej.store.HashStore;
import org.conciergej.store.MasternodeDB;
 ...
 * conciergej for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating conciergej {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use conciergej (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a conciergej context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using conciergej!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing conciergej via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.conciergej.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.conciergej.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.conciergej.core.NetworkParameters} specified when this context was (auto) created. The
