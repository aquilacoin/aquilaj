...
import org.aquilaj.core.listeners.BlockChainListener;
import org.aquilaj.store.FlatDB;
import org.aquilaj.store.HashStore;
import org.aquilaj.store.MasternodeDB;
 ...
 * aquilaj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating aquilaj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use aquilaj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a aquilaj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using aquilaj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing aquilaj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.aquilaj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.aquilaj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.aquilaj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initAquila(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### Aquila init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented aquila MN stuff
